...
namespace CarvedOut
{
    ...
    public class Polygon
    {
        ...
        public List<ClipperLib.IntPoint> ToPath(double xoff = 0, double yoff = 0)
        {
            List<ClipperLib.IntPoint> r = new List<ClipperLib.IntPoint>();

            foreach (var a in Vertices)
            {
                r.Add(new ClipperLib.IntPoint((int)(a.x * 100000 + xoff), (int)(a.y * 100000 + yoff)))
;            }
            return r;
        }
        ...
        class SVGCirc : SVGStyledNode
        {
            ...
            internal void Setup(string xx, string yy, string radius)
            {
                try
                {
                    ...
                    UnTransformed.MakeCircle(x, y, this.radius, 20 * 4);
                }
                catch (Exception)
                {
